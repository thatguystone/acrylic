...
func (ca *contentAnalyze) extractWordCountAndSummary() {
	...
	if len(ca.deets.summary) == 0 {
		...
		if splitAt > -1 {
			ca.deets.summary = ca.content[:splitAt]
		}
	}

	b := bytes.Buffer{}
	z := html.NewTokenizer(strings.NewReader(ca.content))
